...
def easy(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  """Returns loCA reacher with sparse reward with 5e-2 tol."""
  physics = Physics.from_xml_string(*get_model_and_assets())
  task = LoCAReacher(target_size=_BIG_TARGET, random=random)
...
def hard(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  """Returns reacher with sparse reward with 1e-2 tol."""
  physics = Physics.from_xml_string(*get_model_and_assets())
  task = LoCAReacher(target_size=_SMALL_TARGET, random=random)
  environment_kwargs = environment_kwargs or {}
  return control.Environment(
      physics, task, time_limit=time_limit, **environment_kwargs)
...
class Physics(mujoco.Physics):
  """Physics simulation with additional features for the LoCA Reacher domain."""
...
class LoCAReacher(base.Task):
  """A LoCA reacher `Task` to reach the targets."""

  def __init__(self, target_size, random=None):
    """Initialize an instance of `LoCA Reacher`.
